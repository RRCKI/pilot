...
class sshlcgcp2SiteMover(SiteMover.SiteMover):
    ...
    def adler32(filename):
        epic.ssh('python /s/ls2/groups/g0037/bin/adler32cmd.py %s' %(filename))
        return epic.output
    adler32 = staticmethod(adler32)

    def getLocalFileInfo(fname, csumtype="default", date=None):
        epic.ssh('python /s/ls2/groups/g0037/bin/fileinfos.py %s' %(fname))
        pilotErrorDiag=''
        ...
        if csumtype == "adler32":
            ...
            fchecksum = sshlcgcp2SiteMover.adler32(fname)
