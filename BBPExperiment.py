...
class BBPExperiment(Experiment):

    # private data members
    __experiment = "BBP"                   # String defining the experiment
    ...
    def __new__(cls, *args, **kwargs):
        ...
        if not cls.__instance:
            cls.__instance = super(BBPExperiment, cls).__new__(cls, *args, **kwargs)
    ...
    def getBBPRunCommand(self, job, jobSite, trfName):
    ...
    def isBBPRunJob(self, trf):
        if "BBPRunAnaly" in trf or "BBPRunMCProd" in trf:
            return True
        else:
            return False
    ...
    def getJobExecutionCommand(self, job, jobSite, pilot_initdir):
        ...
        isBBPRunJob = self.isBBPRunJob(job.trf)
        tolog("isBBPRunJob = %s " % isBBPRunJob)
        ...
        #    ec, pilotErrorDiag, cmdtrf = self.getBBPRunCommand(job, jobSite, trfName)
	...
	#if job.trf.endswith('.py'):
	#    cmd = "%s %s" % (job.trf, job.jobPars)

        cmd = {"interpreter": "",
    	    "payload": ("%s" % job.trf),
            "parameters": "%s" % job.jobPars }
        
        if job.trf.endswith('.py'):
    	    cmd["interpreter"] = "python"
    	else:
    	    cmd["interpreter"] = "bash"

	# Fix for cmd["enviroment"]
    ...
    def verifySwbase(self, appdir):
        ...
        tolog("BBPExperiment - verifySwbase - nothing to do")

        return 0

    def checkSpecialEnvVars(self, sitename):
        """ Called by pilot.py """
        tolog("BBPExperiment - checkSpecialEnvVars - nothing to do")

        return 0

    def extractAppdir(self):
        """ Called by pilot.py, runMain method """
        tolog("BBPExperiment - extractAppdir - nothing to do")
