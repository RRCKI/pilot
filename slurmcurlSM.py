...
class slurmcurlSiteMover(SiteMover.SiteMover):
    ...
    def adler32(filename):
        if slurmcurlSiteMover.isCloud(filename):
            s,e,fs,sum=slurmcurlSiteMover.getLocalFileInfo(filename,'adler32')
    ...
    def getLocalFileInfo(fname, csumtype="default", date=None):
        ...
        if slurmcurlSiteMover.isCloud(fname):
        ...
        if csumtype == "adler32":
            ...
            fchecksum = slurmcurlSiteMover.adler32(fname)
    ...
    def get_data(self, gpfn, lfn, path, fsize=0, fchecksum=0, guid=0, **pdict):
        ...
        epic.slurm('mkdir -p %s;%s %s -o %s %s/file/%s/fetch' %(path,hpcconf.curl.cmd,hpcconf.curl.args,fullname,hpcconf.curl.server,getfile),walltime=10)
        o=epic.output
        s=epic.exit_code
    ...
    def put_data(self, source, destination, fsize=0, fchecksum=0, **pdict):
        ...
        cmd='%s -X POST %s --header "Content-Type:application/octet-stream" --data-binary @%s %s/file/%s/save' %(hpcconf.curl.cmd,hpcconf.curl.args,source,hpcconf.curl.server,putfile)
